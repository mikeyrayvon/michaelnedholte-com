...
function get_custom_nav(){
	...
	if ( count($first_level_categories) ){
		...
		foreach ( $first_level_categories as $first_level_category ) {
			$first_level_div .= "<li class='menu-item'><a class='first-level parent crumb' data-id='" . $first_level_category->term_id . "'>" . $first_level_category->name . "</a></li>";
			...
			if(count($second_level_categories)){
				...
				foreach ( $second_level_categories as $second_level_category ) {
					$second_level_div[$first_level_category->term_id] .= "<li class='menu-item'><a class='second-level parent crumb' data-id='" . $second_level_category->term_id . "'>" . $second_level_category->name . "</a></li>";
					...
					if(count($third_level_categories)){
						...
						foreach ( $third_level_categories as $third_level_category ) {
							$third_level_div[$second_level_category->term_id] .= '<li class="menu-item"><a href="' . get_term_link( $third_level_category->slug, $third_level_category->taxonomy ) . '" class="js-ajax-item js-menu-item third-level crumb" data-id="' . $third_level_category->term_id . '">' . $third_level_category->name . '</a></li>';
						}
